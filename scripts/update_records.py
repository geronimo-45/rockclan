...
Playwright - page.route로 article API 쿠키 강제 주입
"""
import os, json, re
...
def fetch_article_by_id(context, article_id):
    ...
    def inject_and_capture(route):
        # article API 요청에 쿠키 주입
    ...
    page.route("https://article.cafe.naver.com/**", inject_and_capture)
    ...
    try:
        ...
        xhr_status = captured.get("status", "없음")
        print(f"  [XHR] status={xhr_status}")
